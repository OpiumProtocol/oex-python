...
def create_schema(name, types):
    ...
    dependencyDefinitions = [create_struct_definition(dependency, types[dependency]) for dependency in
                             sorted(dependencies) if types.get(dependency)]
...
def encode_value(dataType, value, types):
    if (dataType == 'string'):
        ...
        return encode_single('bytes32',
                             utils.sha3(b"".join([encode_data(arrayType, arrayValue, types) for arrayValue in value])))
    else:
...
def encode_data(name, data, types):
    return create_schema_hash(name, types) + b"".join(
        [encode_value(schemaType['type'], data[schemaType['name']], types) for schemaType in types[name]])
